  ...
  _initHighlight: function() {
						...
						if (event.originalEvent) {
							if(window.data_type == "block" &&  $(self.highlightEl).hasClass('fullwidth-block') ){
								...
								if(window.data_type == "block" && window.elementHighlight == "block" &&  !($(self.highlightEl).hasClass('fullwidth-block')) ){
									var main = $(self.highlightEl).hasClass('fullwidth-block');
									if ((offset.top  < (y - halfHeight)) || (offset.left  < (x - halfWidth))) {
										if (isIE11) {
		...
		self.frameHtml.on("mouseup touchend", function(event) {
			if (self.isDragging) {
				...
				//	alert('You are not allowed to place your block here.');
					// self.dragElement.replaceWith('');
					// console.log("window.parentSec",window.parentSec.offsetParent);
					...
					if( $(self.highlightEl).hasClass('fullwidth-block') && $($(self.component.html)[0]).data('type') == 'block' ) {
						...
						if(!$(self.highlightEl).hasClass('fullwidth-block') && ( window.elementHighlight != "block")){
							...
							if($(self.highlightEl).hasClass('fullwidth-block') && ( window.elementHighlight == "block")){
								...
								// alert('You have to put this block out of this element.');
								//self.dragElement.replaceWith('');
							}
						}
					}
	...
	_initBox: function() {
		...
		$("#drag-btn").unbind("mousedown").on("mousedown", function(event) {
			...
			var parent = self.highlightEl;
			console.log(window.elementHighlight + ' '+ self.dragElement +' '+ self.highlightEl);
			...
			try {
				if (event.originalEvent) {
					...
					// } else if(element == "block" && window.elementHighlight == "block" &&  !($(self.highlightEl[0]).hasClass('fullwidth-block')) ){
		...
		$("#parent-btn").unbind("click").on("click", function(event) {
			...
			// console.log("parent:######",self.selectedEl);
