				...
				if (self.isDragging) {
					...
					if( $(self.highlightEl[0]).data('type') == 'block' ) {
						$(self.highlightEl[0]).mousemove(function(event){
							...
							var parentSec = $(self.highlightEl[0]).parent().find('section');
							if ( parentSec.children.length > 0 ) {
...
Vvveb.FileManager = {
	...
	addPage: function(name, title, url,pageid) {

		this.pages[name] = {title:title, url:url, pageid:pageid};

		this.tree.append(
			tmpl("vvveb-filemanager-page", {name:name, title:title, url:url, pageid:pageid}));
	},

	addPages: function(pages) {
		for (var page in pages)
		{
			this.addPage(pages[page]['name'], pages[page]['title'], pages[page]['url'], pages[page]['pageid']);
		}
	},
