...
/* controls */
		...
		componentsList.each(function ()
		{
			...
			for (group in Vvveb.ComponentsGroup)
			{
				...
				for (i in components)
				{
					...
					if (component)
					{
						item = $('<li data-section="' + group + '" data-drag-type=component data-type="' + componentType + '" data-search="' + component.name.toLowerCase() + '"><span class="component-icon"><img src="libs/builder/' + component.image + '" alt="" /></span><a class="element-name" href="#">' + component.name + "</a></li>");
		...
		blocksList.each(function () {
			...
			for (group in Vvveb.BlocksGroup) {
				...
				for (i in blocks) {
					...
					if (block)
					{
						item = $('<li data-section="' + group + '" data-drag-type=block data-type="' + blockType + '" data-search="' + block.name.toLowerCase() + '"><span class="block-img"><img src="' + block.image + '" alt="" /></span><a class="element-name" href="#">' + block.name + "</a></li>");
	...
	selectNode:  function(node) {
		...
		if (self.texteditEl && self.selectedEl.get(0) != node) {
			...
			var selectedLeft = jQuery('#select-box').offset().left;
			var leftMargin = selectedLeft - $(window).scrollLeft();
			var leftPos = $('#canvas').width() / 2;

			if ( leftMargin < leftPos ){
				jQuery('#select-actions').css({'left': '0px', 'right': 'auto'});

			} else {
				jQuery('#select-actions').css({'right': '0px', 'left': 'auto'});
			...
			var leftMargin = selectedLeft - $(window).scrollLeft();
			var leftPos = $('#canvas').width() / 2;
			//console.log(leftMargin + ' - ' + leftPos);
		...
		if (target) {
			...
			try {
				...
				var parent_element = y.getElementsByClassName('parent-element');
				for(var i = 0; i < parent_element.length; i++) {
					parent_element[i].classList.remove("parent-element");
				}
				jQuery(self.selectedEl).parent().addClass('parent-element');
  ...
  _initHighlight: function() {
		...
		self.frameHtml.on("mousemove touchmove", function(event) {
			if (event.target && isElement(event.target) && event.originalEvent) {
				...
				window.elementHighlight = self.highlightEl.data('type');
				self.height = 100;
				var offset = target.offset();
				var height = target.outerHeight();
				var ele =  self.highlightEl;
				...
				var y_frame = (frame.contentWindow || frame.contentDocument);
				if (y_frame.document)y_frame = y_frame.document;
				var parent_element = y_frame.getElementsByClassName('error-border');
				var success_element = y_frame.getElementsByClassName('success-border');
				var space_element = y_frame.getElementsByClassName('space');

				for(var i = 0; i < parent_element.length; i++) {
					parent_element[i].classList.remove("error-border");
				}
				for(var i = 0; i < success_element.length; i++) {
					success_element[i].classList.remove("success-border");
				}
				for(var i = 0; i < space_element.length; i++) {
					space_element[i].classList.remove("space");
				}
				...
				if (self.isDragging) {
					...
					if( $(self.highlightEl[0]).data('type') == 'block' ) {
						$(self.highlightEl[0]).mousemove(function(event){
							window.parentSec = $(self.highlightEl[0]).parent().find('section');
							if ( window.parentSec.children.length > 0 ) {
									$(window.parentSec).prev('section').addClass('space');
									// console.log('Yes it has section element');
							} else {
								console.log('It has not any section element');
							}
						});
					}
					console.log("$(self.highlightEl[0])", $(self.highlightEl[0]));
					if(window.data_type == "block" && window.elementHighlight == undefined) {
						$(self.highlightEl[0]).mousemove(function(event){
							let parentNode  = Vvveb.Builder.getSectionElement(self.highlightEl[0]);
							if ( window.parentSec.children.length > 0 ) {
									$(window.parentSec).addClass('space');
									self.highlightEl[0]=window.parentSec[0];
									Vvveb.Builder.afterBeforeForBlock(self,offset,y,x,halfHeight,halfWidth);
									// console.log('Yes it has section element');
							} else {
								console.log('It has not any section element');
							}
					...
					if( (($(self.highlightEl).hasClass('fullwidth-block'))||(window.elementHighlight == "block")) && (self.selectedEl && self.selectedEl.length>0 && $(self.selectedEl[0]).data('type') == 'block') ) {
						self.highlightEl.addClass('success-border');
					} else {
						if(!($(self.highlightEl).hasClass('fullwidth-block'))&&(window.data_type == "component")) {
							self.highlightEl.addClass('success-border');
						}
						if(($(self.highlightEl).hasClass('fullwidth-block'))&&(window.data_type == "component")) {
							// self.highlightEl.addClass('error-border11');
							self.highlightEl.addClass('error-border11');
					...
					try {
						if (event.originalEvent) {
							if(window.data_type == "block" &&  $(self.highlightEl).hasClass('fullwidth-block') ){
										Vvveb.Builder.appendPrependForBlock(self,offset,y,x,halfHeight,halfWidth,parent);
							} else{
								if(window.data_type == "block" && window.elementHighlight == "block" &&  !($(self.highlightEl).hasClass('fullwidth-block')) ){
										Vvveb.Builder.afterBeforeForBlock(self,offset,y,x,halfHeight,halfWidth);
								}else{
										Vvveb.Builder.appendPrependForComponent(self,offset,y,x,halfHeight,halfWidth,parent);
								}
				...
				}// else //uncomment else to disable parent highlighting when dragging
				{
					jQuery("#highlight-box").css({
						...
						//"border":self.isDragging?"8px dashed #777":"",//when dragging highlight parent with green
						"transition":"all 0.3s ease",//when dragging highlight parent with green
					});
		...
		self.frameHtml.on("mouseup touchend", function(event) {
			if (self.isDragging) {
				...
				var relY = event.pageY - $(self.highlightEl[0]).offset().top;
				var eleHeight = self.highlightEl.outerHeight();
				var eleHalfHeight = Math.max(eleHeight / 2, 50);

				self.highlightEl.removeClass('success-border');
				...
				if(window.data_type == "block" && window.elementHighlight == undefined) {
					...
					// alert("(window.data_type == 'block' && window.elementHighlight == undefined)");
							// $(window.parentSec).prev('section').addClass('space');
							// // window.parentSec  = $(self.highlightEl[0]).parent().find('section');
							let parentNode  = Vvveb.Builder.getSectionElement(self.highlightEl[0]);
							// console.log("$(self.component.html):#####:",$(self.component.html));
							// self.highlightEl = window.parentSec[0];
							// newElement = $(self.component.html);
							// self.dragElement.replaceWith(newElement);
							// self.dragElement = newElement;
							// node = self.dragElement.get(0);
							// // node.parentNode=window.parentSec;
							// Vvveb.Builder.insertElement(self,node);
						if (self.dragMoveMutation === false)
						{	
							alert("first if");			
							if (self.component.dragHtml) //if dragHtml is set for dragging then set real component html
							{
								alert("first inner if");	
								newElement = $(self.component.html);
								self.dragElement.replaceWith(newElement);
								self.dragElement = newElement;
							}
						...
						console.log("self.dragElement.get(0);########",self.dragElement.get(0));
						node = self.dragElement.get(0);
						console.log("window.parentSec[0]:########",window.parentSec[0]);
						node.nextSibling =window.parentSec[0];
						...
						if (self.dragMoveMutation === false)
						{
							// alert("second if",{type: 'childList', 
							// target: null, 
							// addedNodes: [node], 
							// nextSibling: window.parentSec[0]});
						...
						{
							// alert("third if");
							self.dragMoveMutation.newParent = null;
							self.dragMoveMutation.newNextSibling = window.parentSec[0];
					...
					// 	self.highlightEl = window.parentSec;
					// 	newElement = $(self.component.html);
					// 	self.dragElement.replaceWith(newElement);
					// 	self.dragElement = newElement;
					// }
					...
					// Vvveb.Builder.insertElement(self,node);
						...
						// alert('adeel na kaha');
					if( $(self.highlightEl).hasClass('fullwidth-block') && $($(self.component.html)[0]).data('type') == 'block' ) {
						if (self.component.dragHtml) //if dragHtml is set for dragging then set real component html
						{
							newElement = $(self.component.html);
							self.dragElement.replaceWith(newElement);
						...
						if(!$(self.highlightEl).hasClass('fullwidth-block') && ( window.elementHighlight != "block")){
							if (self.component.dragHtml) { //if dragHtml is set for dragging then set real component html
								newElement = $(self.component.html);
								self.dragElement.replaceWith(newElement);
								self.dragElement = newElement;
							...
							//window.data_type == "block" && window.elementHighlight == undefined
							if($(self.highlightEl).hasClass('fullwidth-block') && ( window.elementHighlight == "block")){
								if(relY > eleHalfHeight){
									//insert before
									if (self.component.dragHtml) { //if dragHtml is set for dragging then set real component html
										newElement = $(self.component.html);
										self.dragElement.replaceWith(newElement);
		...
		self.frameHtml.on("dblclick", function(event) {
			if (Vvveb.Builder.isPreview == false) {
				...
				jQuery(".draggable-container").hide();
			}
		});
	...
	_initBox: function() {
		...
		$("#drag-btn").unbind("mousedown").on("mousedown", function(event) {
			// alert('sdfsdfsd')
			event.preventDefault();
			// return false;
			...
			window.elementHighlight = self.highlightEl.data('type');
			var element = self.dragElement.data('type');
			...
			try {
				if (event.originalEvent) {
					if(element == 'block' && window.elementHighlight == 'block' && $(self.highlightEl[0]).hasClass('fullwidth-block')) {
						if(relY > eleHalfHeight){
							if (isIE11) {
								self.highlightEl.after(self.dragElement);
							} else {
								$(self.highlightEl).after(self.dragElement);
							}
						} else {
							if (isIE11) {
								self.highlightEl.before(self.dragElement);
							} else {
								$(self.highlightEl).before(self.dragElement);
							}
						}
					} else if(element == undefined && window.elementHighlight != 'block' && !$(self.highlightEl[0]).hasClass('fullwidth-block')) {
						if(relY > eleHalfHeight){
							if (isIE11) {
			...
			} else if(element == undefined && $(self.highlightEl[0]).hasClass('fullwidth-block')) {
				self.dragMoveMutation = false;
			}
			event.preventDefault();
			return false;
		...
		$("#down-btn").unbind("click").on("click", function(event) {
			jQuery("#select-box").hide();
		...
		$("#up-btn").unbind("click").on("click", function(event) {
			jQuery("#select-box").hide();
		...
		$("#clone-element-btn").unbind("click").on("click", function(event) {
			var	clone = jQuery.extend(true, {}, self.selectedEl.clone());
		...
		$("#parent-btn").unbind("click").on("click", function(event) {
			node = self.selectedEl.parent().get(0);
			$("*").removeClass('parent-element');
			self.selectedEl.parent().addClass('parent-element');
		...
		$("#child-btn").unbind("click").on("click", function(event) {
			node = self.selectedEl.children().get(0);
			self.selectNode(node);
			self.loadNodeComponent(node);
			event.preventDefault();
			return false;
		...
		$("#delete-btn").unbind("click").on("click", function(event) {
			jQuery("#select-box").hide();
			node = self.selectedEl.get(0);
			...
			//console.log("self.selectedEl:######",self.selectedEl);
			if(self.selectedEl[0].tagName=='BODY' ||self.selectedEl[0].tagName=='HTML' ||self.selectedEl[0].tagName=='DOCUMENT'){
				console.log("body element we can't delete it");
				alert("Its base element you can't remove it.");
			} else{
				if( $(self.selectedEl[0]).hasClass('fullwidth-block') && $(self.selectedEl[0]).data('type') == 'block' ){
					alert('You are not able to delete this block');
					console.log('it is block');
				} else {
					console.log(self.selectedEl);
		...
		$("#add-section-btn").on("click", function(event) {
			...
			if(addSectionBox.hasClass('open-box')) {
				addSectionBox.removeClass('open-box');

			} else {
				addSectionBox.addClass('open-box');
			}
	...
	_initDragdrop : function() {
		...
		$('.drag-elements-sidepane ul > li > ol > li').unbind("mousedown touchstart").on("mousedown touchstart", function(event) {
			...
			self.dragElement.toggleClass('selected-element');
			...
			if (Vvveb.dragIcon == 'html') {
				...
				self.iconDrag = $('<img src=""/>').attr({"id": "dragElement-clone", 'src': $this.children('span').find('img').attr('src').replace(/^url\(['"](.+)['"]\)/, '$1')});
				$('#dragElement-clone').css({
					'z-index':100,
					'position':'absolute',
					'width':'1px',
					'height':'1px',
			...
			//$('body').append('<div class="dragged-element">' + self.iconDrag + '</div>');
		...
		$('body').on('mousemove touchmove', function(event) {
			if (self.iconDrag && self.isDragging == true) {
				var x = (event.clientX || event.originalEvent.clientX);
				var y = (event.clientY || event.originalEvent.clientY);
				self.iconDrag.css({'left': x - 60, 'top': y - 40});
	...
	getSectionElement : (node)=>{
		let newNode  = $(node).parent();
		if($(newNode).is('section')){
			console.log(" return NODE: ", newNode);
			window.parentSec = newNode;
			return 1;
		}else{
			Vvveb.Builder.getSectionElement(newNode);
		}
	},
	appendPrependForBlock : (self,offset,y,x,halfHeight,halfWidth,parent)=>{
		if ((offset.top  > (y - halfHeight)))  {
			if (isIE11) {
				self.highlightEl.append(self.dragElement);
			} else {
				self.dragElement.appendTo(parent);
	...
	afterBeforeForBlock : (self,offset,y,x,halfHeight,halfWidth)=>{
		if ((offset.top  < (y - halfHeight))) {
			if (isIE11) {
				self.highlightEl.after(self.dragElement);
			} else {
	...
	appendPrependForComponent : (self,offset,y,x,halfHeight,halfWidth,parent)=>{
		if ((offset.top  < (y - halfHeight))) {
			if (isIE11) {
				self.highlightEl.append(self.dragElement);
			} else {
	...
	saveAjax: function(fileName, startTemplateUrl,callback) {
		...
		var data = {};
		data["pageFileName"] = (fileName && fileName != "") ? fileName : Vvveb.FileManager.getCurrentUrl();
...
Vvveb.Gui = {
	...
	saveAjax : function () {
		...
		return Vvveb.Builder.saveAjax(url, null, function (data) {
			$('#message-modal').modal().find(".modal-body").html('<p class="success msg">Congratulations! Your page saved successfully!</p>');
		});
	},
	...
	preview : function () {
		...
		$("#toggle-left-column-btn").fadeToggle();
		$("#toggle-right-column-btn").fadeToggle();
	},
	...
	newPage : function () {
		...
		// newPageModal.modal("show").find("form").off("submit").submit(function( event ) {
			...
			return false;
			// var title = $("input[name=pageTitle]", newPageModal).val();
			// var startTemplateUrl = $("select[name=startTemplateUrl]", newPageModal).val();
			// var fileName = $("input[name=pageFileName]", newPageModal).val();
			//
	...
	createNewPage : function () {
		...
		var data = {};
		data["pageFileName"] = $('#pageFileName').val();
		data["pageTitle"] = $('#pageTitle').val();
		data["startTemplateUrl"] = $('#startTemplateUrl').val();
		data["siteid"] = $('#siteID').val();
		// data["pageid"] = $('#pageID').val();
		// alert('sfsdfdsf');
		$.ajax({
			type: "POST",
			url: 'create-page.php',//set your server side save script url
			data: data,
			cache: false,
			success: function (data) {
				// $('#save-btn').removeAttr('disabled');
				...
				// alert(data);
				console.log(data);
				// var origin   = $('#siteUrl').val();
				// window.location.href = origin+'siteid='+data["siteid"]+'&pageid='+data["pageid"];
				return false;
			},
			error: function (data) {
				alert(data.responseText);
	...
	deletePage : function () {
		var li_element = $('#pages .tree').find('li.active').attr('data-id');
		var origin   = $('#siteUrl').val();
		// alert(li_element);
		 window.location.href = origin+"delete.php?pageid="+li_element;
		 return false;
	},
	...
	//layout
	togglePanel: function (panelparam, cssVar) {
		var panel = $(panelparam);
		var body = $("body");
	...
	toggleLeftColumn: function () {
		...
		$("#vvveb-builder").toggleClass("no-left-panel");
	},
	toggleSettingsPanel: function (e) {
		e.preventDefault();
		$("#settings").toggleClass("active");
		$("#vvveb-builder").toggleClass("settings-panel");
		$("button.settings-icon").toggleClass("active");
	},
	removeSettingsPanel: function (e) {
		e.preventDefault();
		$("#settings").removeClass("active");
		$("#vvveb-builder").removeClass("settings-panel");
		$("button.settings-icon").removeClass("active");
	},
	toggleHistoryPanel: function (e) {
		e.preventDefault();
		$("#history").toggleClass("active");
		$("#vvveb-builder").toggleClass("history-panel");
		$("button.history-icon").toggleClass("active");
	},
	removeHistoryPanel: function (e) {
		e.preventDefault();
		$("#history").removeClass("active");
		$("#vvveb-builder").removeClass("history-panel");
		$("button.history-icon").removeClass("active");
	},
	togglePagesPanel: function (e) {
		e.preventDefault();
		$("#pages").toggleClass("active");
		$("#vvveb-builder").toggleClass("pages-panel");
		$("ul.nav-tabs li.component-pages-tab").toggleClass("active");
	},
	removePagesPanel: function (e) {
		e.preventDefault();
		$("#pages").removeClass("active");
		$("#vvveb-builder").removeClass("pages-panel");
		$("ul.nav-tabs li.component-pages-tab").removeClass("active");
	},
...
Vvveb.FileManager = {
	...
	addPage: function(name, title, url,pageid,classname) {

		this.pages[name] = {title:title, url:url, pageid:pageid, classname:classname};

		this.tree.append(
			tmpl("vvveb-filemanager-page", {name:name, title:title, url:url, pageid:pageid, classname:classname}));
	},

	addPages: function(pages) {
		for (var page in pages)
		{
			this.addPage(pages[page]['name'], pages[page]['title'], pages[page]['url'], pages[page]['pageid'], pages[page]['classname']);
		}
	},
	...
	loadPage: function(name, allowedComponents = false, disableCache = true) {
		...
		var pageidd = $("[data-page='" + name + "']", this.tree).attr("data-id");
		// alert(pageidd);
		$('#currentpageid').val(pageidd);
