...
(function ($) {
    ...
    (function () {
        ...
        $('.dropdown--trigger, .cart-sidebar-btn, .user-menu-btn, .menu-mobile-btn, .wishlist-sidebar-btn').on('click', function (e) {
            closeSearch();
        });
    })();
    ...
    $('.product-add-to-whishlist').on('click', function (e) {
        e.preventDefault();
        var product_id = $(this).attr('id');

        var perv = getCookie('wishlist');

        if(perv !== null) {
            perv = perv + '-' +product_id;
        }else{
            perv = product_id;
        }

        setCookie('wishlist', perv, 6);

        console.log(getCookie('wishlist'));
    });

    (function () {
        var wishSidebarbtn = $(".wishlist-sidebar-btn"),
            wishSidebarMenu = $(".wish-sidebar-menu"),
            cartSidebarClose = $('.close-cart-sidebar'),
            cartBtnActive = 'active',
            cartSidebarActive = 'wish-sidebar-open',
            cartBtnParentActive = 'cart-active';

        var openwishSidebar = function () {
            wishSidebarbtn.addClass(cartBtnActive);
            $body.addClass(cartSidebarActive);
            wishSidebarbtn.parent().addClass(cartBtnParentActive);
        };

        var closeWishSidebar = function () {
            $body.removeClass(cartSidebarActive);
            wishSidebarbtn.removeClass(cartBtnActive);
            wishSidebarbtn.parent().removeClass(cartBtnParentActive);
        };

        wishSidebarbtn.on('click', function (e) {
            e.stopPropagation();
            if ($(this).hasClass(cartBtnActive)) {
                $(this).removeClass(cartBtnActive);
                $body.removeClass(cartSidebarActive);
                ...
                wishSidebarbtn.removeClass(cartBtnActive).parent().removeClass(cartBtnParentActive); $body.removeClass(cartSidebarActive);
                $(this).addClass(cartBtnActive);
                $body.addClass(cartSidebarActive);
                $(this).parent().addClass(cartBtnParentActive);
            }
        ...
        $document.on('click touchstart', function (e) {
            if (!$(e.target).closest(wishSidebarMenu, wishSidebarbtn).length) {
                wishSidebarbtn.removeClass(cartBtnActive);
                $body.removeClass(cartSidebarActive);
                wishSidebarbtn.parent().removeClass(cartBtnParentActive);
                e.stopPropagation();
    ...
    (function () {
        $document.ready(function () {
            $('#newsletter-popup .newsletter-close, #newsletter-popup .newsletter-close-text').on('click', function () {
            ...
            if ($.cookie('visits') > parseInt(1, 10)) {
                $('#active-newsletter-popup').hide();
                $('#newsletter-popup').hide();
            } else {
                var pageHeight = $document.height();
                $('<div id="active-newsletter-popup"></div>').appendTo('body');
                $('#active-newsletter-popup').css("height", pageHeight);
                $('#newsletter-popup').show();
            }

            if ($.cookie('noShowWelcome')) { $('#newsletter-popup').hide(); $('#active-newsletter-popup').hide(); }
        });
